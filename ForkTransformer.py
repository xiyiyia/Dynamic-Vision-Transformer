...
def generate_logits(mlp_model, model, dataloader, T):
    ...
    for i, (x, target, path) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            output.values[output.values >= 8.9056 - 0.125] = 0
            output.values[torch.logical_and(output.values >= 7.9173 - 0.25, output.values < 8.9056 - 0.125)] = 1
            output.values[torch.logical_and(output.values > 2, output.values < 7.9173 - 0.125)] = 2
