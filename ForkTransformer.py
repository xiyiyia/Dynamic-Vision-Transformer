...
def main():
    ...
    if args.eval_mode > 0:
        ...
        train_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_size, num_workers=1,
                                                   pin_memory=False,
        ...
        val_loader = torch.utils.data.DataLoader(imagenet_valid,
                                                 ...
                                                 pin_memory=False, sampler=randomsampler)
        # print(val_loader)
        model = model.cuda()
        ...
        test_logits, test_targets, anytime_classification, ttl = generate_logits(mlp_model, model, val_loader, dynamic_threshold)
...
def generate_logits(mlp_model, model, dataloader, T):
    logits_list = []
    targets_list = []
    ...
    for i, (x, target, path) in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            output.values[output.values >= 8.9056 - 0.01125] = 0
            output.values[torch.logical_and(output.values >= 7.9173 - 0.01125, output.values < 8.9056 - 0.01125)] = 1
            output.values[torch.logical_and(output.values > 2, output.values < 7.9173 - 0.01125)] = 2
            ...
            #     os.system('mv ' + path[0] + ' ./data/val/2/')
            # else:
            #     os.system('mv ' + path[0] + ' ./data/val/3/')
