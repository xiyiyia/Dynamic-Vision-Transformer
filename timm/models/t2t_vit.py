...
class DVT_T2t_vit_14_model(nn.Module):
    ...
    def resize_batch(self, x, features_to_be_reused_list, relations_to_be_reused_list, output, n_stage):
        ...
        features_to_be_reused_list[0] = features_to_be_reused_list[0][max_preds >= self.T[7][n_stage]] #torch.index_select(features_to_be_reused_list[0], 0, indice)
        # print(relations_to_be_reused_list.shape)
        relations_to_be_reused_list = relations_to_be_reused_list[max_preds >= self.T[7][n_stage], :, :, :] # torch.index_select(relations_to_be_reused_list, 0, indice)
...
class DVT_T2t_vit_12_model(nn.Module):
    def __init__(self, feature_reuse, relation_reuse, dynamic_threshold=None, **kwargs):
        ...
        self.feature_reuse = feature_reuse
        self.relation_reuse = relation_reuse
        # print("reuse", feature_reuse, relation_reuse)
        self.T = dynamic_threshold
...
def DVT_T2t_vit_12(dynamic_threshold=None, **kwargs):
    return DVT_T2t_vit_12_model(feature_reuse=True, relation_reuse=True, dynamic_threshold=dynamic_threshold, **kwargs)
